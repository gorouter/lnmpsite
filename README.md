# <center>使用Dockerfile 部署 LNMP+Redis+Ftp 环境 </center>
...
**[Ubuntu linux](https://docs.docker.com/install/linux/docker-ce/ubuntu/#install-docker-ce)**
```
...
#查对KEY是否正确 apt-key fingerprint 0EBFCD88
...
**方法一、 直接使用docker-compose一键制作镜像并运行容器（推荐）**
    ...
    ```
	git clone https://github.com/gorouter/src_dockerfile.git
	cp ./src_dockerfile/lnmpsite ~/lnmpsite
	cd ~/lnmpsite
	docker-compose up -d
    ```
	然后就可以通过访问127.0.0.1、127.0.0.1/index.php、127.0.0.1/db.php、127.0.0.1/redis.php 测试了
	(若需要在ssl文件夹中加入自己的证书文件，本项目自带的是空的，需要自己替换，保持文件名一致)
...
1. nsenter 命令需要安装 (Ubuntu安装后自带)
```
# cd /tmp
# curl https://mirrors.edge.kernel.org/pub/linux/utils/util-linux/v2.31/util-linux-2.31.1.tar.gz | tar -zxf-
